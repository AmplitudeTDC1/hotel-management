...
#====================Backend Functions===========================
#This code defines a function load_menu() that appears to load the menu items into a graphical user interface (GUI) using tkinter.
#The first two lines of the function menuCategory.set("") and menu_tabel.delete(*menu_tabel.get_children()) reset the GUI variables menuCategory 
# and menu_tabel to their initial states.
#The function then reads in a list of files from the "Menu" directory using os.listdir(). For each file in the list, it opens the file using open() 
# and reads the lines of the file in a while loop using f.readline().
#Depending on the contents of the line read in, the function will extract the name, price, and category of the item using string manipulation 
# techniques like line.rfind(). The function then inserts a row in menu_tabel for each item using menu_tabel.insert() with the item name, price, and category.
...
title_label = Label(title_frame, text=" 🙏 🙏* * * *   Sandstorm Resort, Iyana Paja, Lagos. Nigeria  * * * *     🙏 🙏", 
